...
export class RankingController {
  ...
  @ApiOperation({
    summary: '주간 전체 랭킹 조회',
    description: '전체 사용자 기준으로 주간 전체 랭킹을 반환한다.',
  })
  @ApiOkResponse({
    description: '주간 전체 랭킹 조회 성공',
  })
  @UseGuards(JwtAccessGuard)
  async getOverallWeeklyRanking(
    ...
    const result = await this.rankingQueryService.getOverallWeeklyRanking(userId, weekKey ?? null);

    return {
      result,
      message: '주간 전체 랭킹을 조회했습니다.',
    };
  }
